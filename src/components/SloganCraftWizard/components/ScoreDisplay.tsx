...
export function ScorePanel({ scores }: { scores: SloganScores }) {
  return (
    <div className="space-y-1.5">
      {(Object.keys(scores) as (keyof SloganScores)[]).map((key) => (
        <ScoreBar key={key} label={SCORE_LABELS[key]} value={scores[key]} />
      ))}
    </div>
  );
}
